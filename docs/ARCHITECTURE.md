...
The provisional and deterministic low-health fallbacks select `RETREAT` while it is legal; if retreat is temporarily unavailable they select `HOLD_POSITION` instead of combat or objective movement. `ValidPlanFactory` omits RETREAT once `RetreatPlanner` reports the robot sufficiently safe. On first entering that safe state, `RobotController` stops navigation and emits one urgent `RETREAT_SAFE` redecision. Because the next candidate set no longer contains RETREAT, low-confidence composition cannot indefinitely retain a completed retreat; low-health robots can HOLD to recover and later decisions can return them to active plans.
