...
`core/CoreController.java` owns capture occupancy, team ownership, telemetry, and particle boundary. The configured `arena.core.radius` is the half-width/depth of the X/Z CORE square; `isInsideBounds` also checks Y distance. The visual perimeter is built from the same X/Z bounds. Robot movement and target validity use the separate circular `arena.radius` boundary. `CAPTURE_CORE` uses the same `CoreController.isInside` bounds as its arrival condition: once a robot is inside the CORE it stops pushing toward the exact center. While an enemy robot occupies those CORE bounds, the capturing robot temporarily pursues the nearest CORE occupant and attacks when in range, then resumes objective occupancy when the contest clears. CORE hold scoring is KOTH-style: the current owner gains hold ticks and periodic hold points only while that same team has sole CORE occupancy. EMPTY, CONTESTED, or opponent-only occupancy resets `nextHoldScoreTick` to `-1`, so the owner must regain sole control and complete a fresh hold interval before scoring again. Ownership itself remains until another team completes capture. `combat/` tracks damage and kill resolution; `robot/` creates and tracks robot entities and respawns; `match/ScoreState` and `TeamScoreState` hold personal and team scoring.
...
Provisional plans are latency-hiding behavior only and must not alter authoritative reachability. `RESPAWN` decisions provisionally use `HOLD_POSITION`, and provisional HOLD is stationary while the request is pending. If a provisional plan encounters a movement failure, `RobotController` clears the temporary plan, marks provisional behavior suppressed for that pending request, and does not write to `PlanReachabilityTracker` or `TargetReachabilityTracker`. `JevDecisionService` will not assign another provisional plan until that request completes, so provisional failure cannot remove candidates or create a retry loop before the real response is applied.

Team tactics add `HOLD_POSITION` and `ASSIST_<ALLY>` as separate tactical plans. HOLD captures an anchor position when the plan is applied and runs for a bounded four-second window. It never pursues an enemy: local self-defense is limited to the smaller of `robot.hold-reaction-range` and the 2-block manual attack range, and the default reaction range is 2 blocks. If displaced while no melee threat is present, the controller navigates back to the anchor instead of following the enemy away. HOLD then emits one `HOLD_COMPLETE`. High-confidence AI may deliberately apply a fresh HOLD and reset the window, but `JevDecisionService` keeps an already-expired HOLD unchanged when fallback would choose HOLD again, preventing fallback/provisional HOLD from continually resetting its timer. `TypeSafeJevClient` defines HOLD as a non-default intent that requires positive waiting evidence from Doctrine plus current state; uncertainty, current-plan continuity, or a negative constraint on another action (for example, "do not chase") must not by themselves produce HOLD. Active Doctrine preferences for combat, CORE control, or ally support map to FIGHT, CONTROL_CORE, or SUPPORT instead. ASSIST validates a living same-team ally in the same match, rejects retreat targets and assist cycles, supports the ally's active combat target when legal, and otherwise maintains a two-to-four-block support distance. `RobotStateSerializer` creates dedicated ally snapshots with `current_plan`, `combat_target`, `inside_core`, and `under_attack`; `TypeSafeJevClient` serializes those fields and asks for `ally_target` only when more than one ASSIST candidate is legal. Decision logs use schema version 4 and include the ally target choice.
